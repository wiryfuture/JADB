    ...
    async execute(client, message, args) {
        ...
        if (Math.floor(deleteCount / 100)) {
            ...
            for (let x = 0; x < Math.floor(deleteCount / 100); x++) {
                ...
                await message.channel.bulkDelete(fetched).catch(error => message.reply(`Couldn't delete messages because of: ${error}`)).then(message => {message.delete({timeout: 10000})})
            }
        }
